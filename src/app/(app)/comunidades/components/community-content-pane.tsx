...
export function CommunityContentPane({
  ...
  function shouldSuppressCardOpen() {
    return Date.now() < suppressCardOpenUntilRef.current;
  }

  function suppressCardOpenForAction() {
    suppressCardOpenUntilRef.current = Date.now() + 300;
  }

  const renderPublicationItem = (item: CommunityFeedItem) => {
    const likeSummary = item.reactions?.find(
    ...
    return (
      ...
      <article
        key={item.id}
        className="relative overflow-hidden rounded-lg border border-border bg-background transition-shadow shadow-xs hover:shadow-md hover:border-gray-300">
        {/* Área de clique na puplicação */}
        <div
          role="button"
          tabIndex={0}
          className="cursor-pointer"
          onClick={() => {
            if (shouldSuppressCardOpen()) return;
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  ...
                  variant="ghost"
                  size="icon-sm"
                  className="absolute right-3 top-3 z-10 cursor-pointer bg-secondary/40 backdrop-blur"
              ...
              showFeedItems ? (
                ...
                <Empty className="w-full max-w-3xl border-0">
                  <EmptyHeader className="gap-4">
                    ...
                    <EmptyDescription>
                      O conteúdo publicado neste espaço aparecerá aqui e também
                      no feed consolidado da comunidade.
                    </EmptyDescription>
                  </EmptyHeader>
                  {communityDetail.canPost ? (
                    <EmptyContent>
              ...
              <Empty className="w-full max-w-3xl border-0">
                <EmptyHeader>
                  ...
                  <EmptyDescription>
                    ...
                    experiência de eventos continua como placeholder nesta
                    versão.
                  </EmptyDescription>
                </EmptyHeader>
              </Empty>
            ...
            <Empty className="w-full max-w-3xl border-0">
              ...
              <EmptyContent>
                {publicationSpace && communityDetail.canPost ? (
                  <Button
                    onClick={() => onNavigateToSpace(publicationSpace.id)}>
                    Criar a primeira publicação
                  </Button>
                ) : canManage ? (
                  <Button
