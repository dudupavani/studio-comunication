...
import { fetchChats } from "@/lib/helpdesk/queries";
import {
  errorResponse,
  handleRouteError,
  parsePagination,
} from "@/lib/helpdesk/api-helpers";
import { createChatSchema } from "@/lib/helpdesk/validations";
  ...
  try {
    ...
    if (membersPayload.length > 0) {
      ...
      if (membersError && membersError.code !== "23505") {
        console.error("HELPDESK create members error:", membersError);
