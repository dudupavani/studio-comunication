...
export async function POST(
  ...
  try {
    ...
    if (existingError && existingError.code !== "PGRST116") {
      console.error("HELPDESK fetch member error:", existingError);
    ...
    if (existing) {
      ...
      if (updateError) {
        console.error("HELPDESK update member error:", updateError);
      ...
      if (insertError && insertError.code !== "23505") {
        console.error("HELPDESK add member error:", insertError);
