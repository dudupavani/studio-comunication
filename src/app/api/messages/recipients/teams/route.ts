...
export async function GET(req: NextRequest) {
  try {
    ...
    if (error) {
      console.error("MESSAGES recipients teams error:", error);
      return errorResponse(500, "db_error", "Failed to load teams");
    }
    ...
    const items = slice.map((row: any) => ({
      ...
      membersCount: Array.isArray(row.equipe_members)
        ? row.equipe_members[0]?.count ?? 0
        : 0,
    }));
